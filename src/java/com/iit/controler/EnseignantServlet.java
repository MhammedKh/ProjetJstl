    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            EnseignantADO ensADO = new EnseignantADO();
             ArrayList<Enseignant> ensList = new ArrayList<Enseignant>();
             ensList= ensADO.listEns(); 
             out.println(ensList.get(0).getCin());
            request.setAttribute("ensList", ensList);
             RequestDispatcher rd = getServletContext().getRequestDispatcher("/enseignant.jsp");
